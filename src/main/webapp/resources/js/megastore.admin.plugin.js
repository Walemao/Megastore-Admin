$(function() {
	...
	$('#startDateChoose').click(function() {
		...
		endDate : $('#endDateChoose').val()
			...
			'changeDate',
			function(ev) {
				$('#endDateChoose').datetimepicker('setStartDate',
	...
	$('#endDateChoose').click(function() {
		...
		startDate : $('#startDateChoose').val()
			...
			'changeDate',
			function(ev) {
				$('#startDateChoose').datetimepicker('setEndDate',
						new Date(ev.date.valueOf()));
	...
	$('.ranges li').click(
			function() {
				...
				if ($(this).hasClass('7days')) {
					start_date = new Date(current_date.getTime() - 7 * 24
							* 3600 * 1000);
				}
				if ($(this).hasClass('30days')) {
					start_date = new Date(current_date.getTime() - 30 * 24
							* 3600 * 1000);
				}
				if ($(this).hasClass('60days')) {
					start_date = new Date(current_date.getTime() - 60 * 24
							* 3600 * 1000);
				}
				if ($(this).hasClass('90days')) {
					start_date = new Date(current_date.getTime() - 90 * 24
							* 3600 * 1000);
				}
				...
				$('#startDateChoose')
						.datetimepicker('setEndDate', current_date);
...
function setDataTableSearchInput() {
	...
	var table_search_span = $('<span/>').addClass('input-group-addon').html(
...
function appendProductColor(thumbnail, name, thummd5, id) {
	var p = $('<p/>').addClass('form-control-static').addClass('clearfix')
			.attr('data-str', thummd5).attr('data-id', id);
	var radio = $('<input/>').attr('type', 'radio').attr('name', 'mainImg')
			.addClass('main-img-radio').val(thumbnail).attr('data-toggle',
					'tooltip').attr('data-original-title', '设为主图');
	var a = $('<a/>').addClass('thumbnail').attr('data-toggle', 'modal').attr(
			'data-target', '#product-color-modal-update');
	var img = $('<img/>').attr('width', '50').attr('height', '50').attr('src',
			thumbnail);
	var color_span = $('<span/>').addClass('color-text').text(name);
	var delete_span = $('<span/>').addClass('color-delete').attr('data-toggle',
			'tooltip').attr('data-original-title', '删除').html(
			'<i class="icon-trash"></i>').attr('onclick',
			'deleteInfo(' + id + ')');
	var input = $('<input/>').attr('type', 'hidden').attr('name', 'colorId')
			.val(id);
	a.append(img);
	p.append(radio).append(a).append(color_span).append(delete_span).append(
			input);
